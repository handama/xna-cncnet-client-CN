    ...
    public class CnCNetManager : IConnectionManager
    {
        ...
        public CnCNetManager(WindowManager wm, GameCollection gc)
        {
            ...
            ircChatColors = new IRCColor[]
            {
                ...
                new IRCColor("柠檬绿", true, Color.Lime, 9),
                ...
                new IRCColor("浅灰色", true, Color.LightGray, 14),
                new IRCColor("灰色", false, Color.Gray, 15)
            };
        }
        ...
        private void DoChatMessageReceived(string receiver, string senderName, string ident, string message)
        {
            ...
            if (message.Contains("ACTION"))
            {
                ...
                for (int i = 0; i < gameCollection.GameList.Count; i++)
                    message = message.Replace("新 " + gameCollection.GetGameIdentifierFromIndex(i) + " 游戏",
                        "新 " + gameCollection.GetFullGameNameFromIndex(i) + " 游戏");
        ...
        private void DoConnectAttemptFailed()
        {
            ...
            MainChannel.AddMessage(new ChatMessage(Color.Red, "无法连接至CnCNet！"));
        }
        ...
        private void DoConnected()
        {
            ...
            MainChannel.AddMessage(new ChatMessage("成功连接至CnCNet。"));
        }
        ...
        private void DoConnectionLost(string reason)
        {
            ...
            MainChannel.AddMessage(new ChatMessage(Color.Red, "到CnCNet的连接丢失了。"));
            connected = false;
        }
        ...
        {
            disconnect = false;
            MainChannel.AddMessage(new ChatMessage("连接至CnCNet..."));
        ...
        private void DoDisconnected()
        {
            ...
            MainChannel.AddMessage(new ChatMessage("你从CnCNet离线了。"));
            connected = false;
        ...
        private void DoReconnectAttempt()
        {
            ...
            MainChannel.AddMessage(new ChatMessage("尝试重新连接至CnCNet..."));
        ...
        private void DoNameAlreadyInUse()
        {
            ...
            MainChannel.AddMessage(new ChatMessage(Color.White,
                string.Format("你的名字已经被占用，正在使用 {0} 重新尝试...", sb.ToString())));
